...
-- Fixes Supabase Auth digit-only phone representation vs project +E164 storage.
...
begin
  ...
  for v_item in select value from jsonb_array_elements(p_rows) loop
    ...
    select
      case
        when nullif(btrim(auth_user.phone), '') is null then null
        when btrim(auth_user.phone) ~ '^\+[1-9][0-9]{7,14}$' then btrim(auth_user.phone)
        when btrim(auth_user.phone) ~ '^[1-9][0-9]{7,14}$' then '+' || btrim(auth_user.phone)
    ...
    if v_auth_phone <> v_phone then
      raise exception 'Canonical Auth phone does not match prepared contact for player %.', v_player_id;
    end if;
