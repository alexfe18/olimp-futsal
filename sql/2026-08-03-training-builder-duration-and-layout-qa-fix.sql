-- Sprint 05.0.2 — Training Builder duration/save QA fix
-- Run this file after Sprint 05.0 / 05.0.1 migrations.
-- It calculates planned_duration before creating the training_plans row,
-- so database CHECK constraints never receive the temporary value 0.
...
begin
  ...
  -- Calculate the real duration before inserting the parent plan.
  -- The previous function inserted planned_duration = 0 and only recalculated
  -- it after inserting blocks, which violated training_plans_planned_duration_check.
  select coalesce(
    sum(
      greatest(
        1,
  ...
  -- Recalculate from persisted blocks as a final consistency check.
