...
export default function EditNewsPage() {
  ...
  const [originalNews, setOriginalNews] = useState<NewsRow | null>(null);

  const [slugWasEdited, setSlugWasEdited] = useState(true);
  ...
  async function loadNews(showLoader = false) {
    ...
    if (error) {
      ...
      setMessage("Не вдалося завантажити новину.");
  ...
  const hasChanges = useMemo(() => {
    if (!form || !originalNews) {
      return false;
    }

    return (
      form.title !== originalNews.title ||
      form.slug !== originalNews.slug ||
      form.excerpt !== (originalNews.excerpt ?? "") ||
      form.content !== originalNews.content ||
      form.coverUrl !== (originalNews.cover_url ?? "") ||
      form.status !== originalNews.status ||
      form.isFeatured !== originalNews.is_featured
    );
  }, [form, originalNews]);
  ...
  async function uploadCover(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const filePath = `covers/${newsId}/${Date.now()}-${crypto.randomUUID()}.${extension}`;
      ...
      const previousPath = form.coverUrl
        ? getStoragePathFromPublicUrl(form.coverUrl)
        : null;
  ...
  async function saveNews(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setMessage(
        error instanceof Error ? error.message : "Не вдалося оновити новину.",
      );
      setMessageType("error");
    } finally {
      setIsSaving(false);
  ...
  async function deleteNews() {
    ...
    const confirmed = window.confirm(
      `Видалити новину «${originalNews.title}»? Цю дію неможливо скасувати.`,
    );
    ...
    try {
      if (form?.coverUrl) {
        ...
        if (filePath) {
          ...
          if (storageError) {
            console.warn("News cover removal warning:", storageError);
          }
        }
      }
      ...
      setMessage(
        error instanceof Error ? error.message : "Не вдалося видалити новину.",
      );
      setMessageType("error");
      setIsDeleting(false);
    }
  ...
  if (!form || !originalNews) {
    return (
      <div className="rounded-[2rem] border border-red-200 bg-red-50 p-8">
        <h1 className="text-2xl font-black text-red-800">Новину не знайдено</h1>
        ...
        <div className="mt-3 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-300 sm:text-base">
              Оновлюйте текст, обкладинку, статус публікації та закріплення
              новини.
            </p>
          </div>
        ...
        <div className="space-y-6">
          ...
          <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
            ...
            <div className="mt-5 grid gap-3 rounded-2xl bg-slate-50 p-4 text-sm text-slate-600 sm:grid-cols-2">
              <p>
                <strong className="text-slate-800">Створено:</strong>{" "}
                {formatDate(originalNews.created_at)}
              </p>
